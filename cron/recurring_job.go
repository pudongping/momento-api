...
func (j *RecurringJob) processRecurring(recurring *model.RecurringTransactions, nowTimestamp uint64) {
	...
	if recurring.NextExecutionTime == 0 {
		...
		updateData := map[string]interface{}{
			"next_execution_time": recurring.NextExecutionTime,
		}
		if _, err := j.svcCtx.RecurringTransactionsModel.UpdateFilter(j.ctx, nil, updateData, squirrel.Eq{"recurring_id": recurring.RecurringId}); err != nil {
			logx.Errorf("更新周期性记账任务 %d 初始化时间失败: %v", recurring.RecurringId, err)
		}
		return
	}
	...
	for currentExecutionTime <= nowTimestamp && catchUpCount < maxCatchUp {
		...
		if err := j.executeTransaction(recurring, currentExecutionTime); err != nil {
			logx.Errorf("执行周期性记账任务 %d 在 %d 失败: %v", recurring.RecurringId, currentExecutionTime, err)
	...
	recurring.LastExecutedAt = updateTime // 记录最后一次处理的时间
	recurring.UpdatedAt = updateTime
	...
	if _, err := j.svcCtx.RecurringTransactionsModel.UpdateFilter(j.ctx, nil, updateData, squirrel.Eq{"recurring_id": recurring.RecurringId}); err != nil {
		logx.Errorf("更新周期性记账任务 %d 下次执行时间失败: %v", recurring.RecurringId, err)
	}
}
...
func (j *RecurringJob) executeTransaction(recurring *model.RecurringTransactions, executionTime uint64) error {
	...
	newTransaction := &model.Transactions{
		...
		IsAutoGenerated: model.TransactionIsAutoGeneratedYes, // 是自动生成
