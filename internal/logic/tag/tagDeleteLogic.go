...
func (l *TagDeleteLogic) TagDelete(req *types.TagDeleteReq) error {
	...
	if err != nil {
		...
		return errcode.DBError.WithError(errors.Wrapf(err, "TagDelete FindOne tagID : %d", req.TagId)).Msgr("标签不存在")
	}

	// 检查标签是否为系统标签，不允许删除系统标签
	if tag.IsSystem == model.TagsIsSystemYes {
		return errcode.Fail.Msgr("系统标签不允许删除")
	}

	// 检查标签是否属于当前用户，不允许删除其他用户的标签
	if tag.UserId != userIDUint {
		return errcode.Fail.Msgr("没有权限删除该标签")
	}
	...
	if _, err := l.svcCtx.TagsModel.DeleteFilter(l.ctx, nil, where); err != nil {
		...
		return errcode.DBError.WithError(errors.Wrapf(err, "TagDelete DeleteFilter tagID : %d", req.TagId)).Msgr("删除标签失败")
	}
