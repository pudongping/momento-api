...
// 检查节日是否存在且属于当前用户
func (s *FestivalService) CheckFestivalOwnership(userID uint64, festivalID int64, operationType string) (*model.Festivals, error) {
	festival, err := s.svcCtx.FestivalsModel.FindOne(s.ctx, cast.ToUint64(festivalID))
	if err != nil {
		s.Logger.Errorf("查询节日失败 userID : %d, festivalID : %d, err : %v", userID, festivalID, err)
...
func (s *FestivalService) IsValidAndFutureFestivalDate(festivalDate int64) bool {
