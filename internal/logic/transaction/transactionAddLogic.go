...
func (l *TransactionAddLogic) TransactionAdd(req *types.TransactionAddReq) (resp *types.TransactionAddResp, err error) {
	...
	if req.IsRecurringEnabled {
		...
		return &types.TransactionAddResp{
			TransactionItem: types.TransactionItem{
				TransactionId:   cast.ToString(lastId),
				BookId:          req.BookId,
				...
				TagIcon:         tag.Icon,
				Remark:          req.Remark,
				TransactionTime: 0, // 周期性记账规则本身没有交易时间
				CreatedAt:       now,
				UpdatedAt:       now,
				IsAutoGenerated: 1, // 标记为自动生成
			},
		}, nil
