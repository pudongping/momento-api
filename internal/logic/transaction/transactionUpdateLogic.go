...
func (l *TransactionUpdateLogic) TransactionUpdate(req *types.TransactionUpdateReq) (resp *types.TransactionUpdateResp, err error) {
	...
	// 重新查询关联信息以返回完整结构 (主要是 Tag 信息)
	tag, err := l.svcCtx.TagsModel.FindOne(l.ctx, transactionData.TagId)
	if err != nil {
		// 理论上不应该发生，除非标签被删除了
		return nil, errcode.DBError.Msgr("查询标签失败")
	}

	return &types.TransactionUpdateResp{
		TransactionItem: types.TransactionItem{
			TransactionId:   cast.ToString(transactionData.TransactionId),
			BookId:          int64(transactionData.BookId),
			UserId:          cast.ToString(transactionData.UserId),
			Type:            transactionData.Type,
			Amount:          transactionData.Amount,
			TagId:           int64(transactionData.TagId),
			TagName:         tag.Name,
			TagColor:        tag.Color,
			TagIcon:         tag.Icon,
			Remark:          transactionData.Remark,
			TransactionTime: int64(transactionData.TransactionTime),
			CreatedAt:       int64(transactionData.CreatedAt),
			UpdatedAt:       int64(transactionData.UpdatedAt),
			IsAutoGenerated: transactionData.IsAutoGenerated,
		},
	}, nil
}
