...
// 获取周期性记账列表
func RecurringListHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.RecurringListReq
		...
		// 参数校验
		if msg, ok := validator.CallValidate(&req, requests.RecurringListRequestCheck); !ok {
			responses.ToParamValidateResponse(r, w, nil, msg)
			return
		}

		l := recurring.NewRecurringListLogic(r.Context(), svcCtx)
		resp, err := l.RecurringList(&req)
